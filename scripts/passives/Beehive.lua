...
pExodus.ItemId.BEEHIVE = Isaac.GetItemIdByName("Beehive")
pExodus.CostumeId.BEEHIVE = Isaac.GetCostumeIdByPath("gfx/characters/costume_Beehive.anm2")
			...
			if entity.Type == EntityType.ENTITY_TEAR then
				if entity.SpawnerType == EntityType.ENTITY_PLAYER or entity.SpawnerType == EntityType.ENTITY_FAMILIAR then
					if pExodus.CompareEntities(entity.Parent, player) or entity.SpawnerVariant == 120 or entity.SpawnerVariant == 80 then
						...
						if player:HasCollectible(CollectibleType.COLLECTIBLE_MARKED) and rng:RandomInt(6) == 0 then
							for u, effect in pairs(pExodus.RoomEntities) do
...
pExodus:AddCallback(ModCallbacks.MC_POST_UPDATE, pExodus.beehiveUpdate)