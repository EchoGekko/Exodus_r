...
pExodus.ItemId.DRAGON_BREATH = Isaac.GetItemIdByName("Dragon Breath")
pExodus.CostumeId.DRAGON_BREATH = Isaac.GetCostumeIdByPath("gfx/characters/costume_Dragon Breath.anm2")
	...
	if player:HasCollectible(ItemId.DRAGON_BREATH) then
		...
		for i, entity in pairs(Isaac.GetRoomEntities()) do
			...
			if entity.Type == Entities.FIREBALL.id and entity.Variant == Entities.FIREBALL.variant then
				...
				if rng:RandomInt(4) == 0 and entity:ToTear().TearFlags & TearFlags.TEAR_LUDOVICO == 0 then
...
pExodus:AddCallback(ModCallbacks.MC_POST_UPDATE, pExodus.dragonBreathUpdate)  
...
pExodus:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, pExodus.dragonBreathCache, CacheFlag.CACHE_FIREDELAY + CacheFlag.CACHE_DAMAGE)